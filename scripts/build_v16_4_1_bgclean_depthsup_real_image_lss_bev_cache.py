...
class Config:
    ...
    out_dir: str = "/home/ubuntu22/decision_on_nuscenes/outputs_v16_4_1_highres_depthsup_bgclean_real_image_lss_bev_cache"
    ...
    # v16.4.1 default is deliberately harsher on false positives.
    # Your preview showed ray-like hallucination, so foreground weight is reduced
    # and background suppression gets its own loss instead of relying on BCE/Dice.
    lambda_bce: float = 0.30
    lambda_focal: float = 0.45
    lambda_dice: float = 0.35
    lambda_edge: float = 0.90
    lambda_boundary: float = 0.80
    lambda_sobel: float = 0.45
    lambda_thin: float = 0.60
    lambda_smooth: float = 0.02
    lambda_bg_far: float = 0.85
    lambda_bg_all: float = 0.20
    focal_gamma: float = 2.0
    foreground_pos_weight: float = 3.0

    # Hard background suppression: penalize teacher=0 / pred>0, especially far from teacher foreground.
    bg_target_thresh: float = 0.05
    bg_pred_margin: float = 0.06
    bg_far_dilate_kernel: int = 11
    bg_hard_weight: float = 4.0

    # Thin lane / small-object emphasis. This prevents the big drivable-looking blob from dominating.
    thin_density_kernel: int = 7
    thin_density_max: float = 0.42
    thin_extra_weight: float = 12.0

    # Conservative cache cleaning for planner-side ablation.
    export_cleaned_bev: bool = True
    clean_low_threshold: float = 0.12
    clean_low_scale: float = 0.25
    clean_zero_threshold: float = 0.05

    # v16.4: LiDAR sparse depth supervision, with local densification.
    lambda_depth: float = 0.80
...
def cfg_from_env() -> Config:
    ...
    c.lambda_bg_far = float(os.getenv("LAMBDA_BG_FAR", c.lambda_bg_far))
    c.lambda_bg_all = float(os.getenv("LAMBDA_BG_ALL", c.lambda_bg_all))
    ...
    c.bg_target_thresh = float(os.getenv("BG_TARGET_THRESH", c.bg_target_thresh))
    c.bg_pred_margin = float(os.getenv("BG_PRED_MARGIN", c.bg_pred_margin))
    c.bg_far_dilate_kernel = int(os.getenv("BG_FAR_DILATE_KERNEL", c.bg_far_dilate_kernel))
    c.bg_hard_weight = float(os.getenv("BG_HARD_WEIGHT", c.bg_hard_weight))
    c.thin_density_kernel = int(os.getenv("THIN_DENSITY_KERNEL", c.thin_density_kernel))
    c.thin_density_max = float(os.getenv("THIN_DENSITY_MAX", c.thin_density_max))
    c.thin_extra_weight = float(os.getenv("THIN_EXTRA_WEIGHT", c.thin_extra_weight))
    c.export_cleaned_bev = os.getenv("EXPORT_CLEANED_BEV", "1" if c.export_cleaned_bev else "0").lower() in {"1", "true", "yes", "y", "on"}
    c.clean_low_threshold = float(os.getenv("CLEAN_LOW_THRESHOLD", c.clean_low_threshold))
    c.clean_low_scale = float(os.getenv("CLEAN_LOW_SCALE", c.clean_low_scale))
    c.clean_zero_threshold = float(os.getenv("CLEAN_ZERO_THRESHOLD", c.clean_zero_threshold))
...
def thin_structure_loss(pred, target, mask, cfg: Config):
    """Extra supervision for lane lines and small objects.

    v16.4 already had a thin loss, but it still treated every foreground cell as thin.
    That lets the big center blob dominate. v16.4.1 detects low local occupancy
    foreground and teacher edges, then gives those cells much larger gradients.
    """
    target_any = target.max(dim=1, keepdim=True)[0]
    local_density = F.avg_pool2d(
        (target_any > 0.10).float(),
        kernel_size=int(cfg.thin_density_kernel),
        stride=1,
        padding=int(cfg.thin_density_kernel) // 2,
    )
    thin_spatial = ((target_any > 0.15) & (local_density < float(cfg.thin_density_max))).float()
    edge_spatial = dilate2d((edge_map(target_any) > 0.05).float(), k=3)
    thin_band = torch.clamp(thin_spatial + edge_spatial, 0, 1)
    weight = 1.0 + float(cfg.thin_extra_weight) * thin_band
    raw = F.binary_cross_entropy(pred.clamp(1e-4, 1 - 1e-4), target, reduction="none")
    denom = (weight * mask).sum().clamp_min(1.0)
    return (raw * weight * mask).sum() / denom


def hard_background_suppression_loss(pred, target, mask, cfg: Config):
    """Punish hallucinated BEV energy where the teacher is clear background.

    This directly targets the ray/fan-shaped false positives in your preview:
        teacher == 0 but pred is green/yellow.

    far background is weighted harder than near-boundary background so the model
    can still tolerate one-cell geometric mismatch around true lanes/objects.
    """
    target_any = target.max(dim=1, keepdim=True)[0]
    fg_near = dilate2d((target_any > float(cfg.bg_target_thresh)).float(), k=int(cfg.bg_far_dilate_kernel))
    far_bg_spatial = (fg_near < 0.5).float()
    channel_bg = (target < float(cfg.bg_target_thresh)).float()

    excess = torch.relu(pred - float(cfg.bg_pred_margin))
    hard = 1.0 + float(cfg.bg_hard_weight) * (pred.detach() > max(float(cfg.clean_low_threshold), float(cfg.bg_pred_margin))).float()

    far_weight = far_bg_spatial * channel_bg * mask
    all_weight = channel_bg * mask
    far_loss = ((excess ** 2) * hard * far_weight).sum() / far_weight.sum().clamp_min(1.0)
    all_loss = ((pred ** 2) * all_weight).sum() / all_weight.sum().clamp_min(1.0)
    return far_loss, all_loss


def clean_bev_prediction_np(bev: np.ndarray, cfg: Config) -> np.ndarray:
    """Conservative planner-side cleaned BEV cache.

    This is not used for training. It gives you raw-vs-cleaned ablation without
    rebuilding the network. Low-confidence fog is damped/zeroed, high-confidence
    foreground is preserved.
    """
    x = np.asarray(bev, dtype=np.float32).copy()
    x[x < float(cfg.clean_low_threshold)] *= float(cfg.clean_low_scale)
    x[x < float(cfg.clean_zero_threshold)] = 0.0
...
def compute_loss(pred, target, valid, cfg: Config):
    ...
    bg_far, bg_all = hard_background_suppression_loss(pred, target, mask, cfg)
    ...
    loss = (
        ...
        + cfg.lambda_bg_far * bg_far
        + cfg.lambda_bg_all * bg_all
...
def train_epoch(model, loader, opt, cfg: Config, epoch: int):
    model.train()
    total, n = 0.0, 0
    parts_sum = {"bce": 0.0, "focal": 0.0, "dice": 0.0, "edge": 0.0, "boundary": 0.0, "sobel": 0.0, "thin": 0.0, "bg_far": 0.0, "bg_all": 0.0, "smooth": 0.0, "depth": 0.0, "depth_pts": 0.0}
    for i, batch in enumerate(loader, 1):
        ...
        for k, v in parts.items():
            parts_sum[k] += v * bs
        if i % 20 == 0:
            print(f"[train e{epoch:03d} i{i:04d}] loss={loss.item():.5f} depth={parts['depth']:.5f} pts={parts['depth_pts']:.1f} bce={parts['bce']:.5f} focal={parts['focal']:.5f} dice={parts['dice']:.5f} edge={parts['edge']:.5f} boundary={parts['boundary']:.5f} sobel={parts['sobel']:.5f} thin={parts['thin']:.5f} bg_far={parts['bg_far']:.5f} bg_all={parts['bg_all']:.5f}")
...
def export_split(model, ds, cfg: Config, split: str, out_dir: str) -> List[str]:
    ...
    model.eval()
    files = []
    toks, bevs, bevs_clean, valids = [], [], [], []
    shard = 0

    def flush():
        nonlocal toks, bevs, bevs_clean, valids, shard
        if not toks:
            return
        ...
        if bool(cfg.export_cleaned_bev):
            payload["image_bev_feat_clean"] = np.stack(bevs_clean).astype(np.float32)
        np.savez_compressed(path, **payload)
        print(f"[export {split}] shard={shard:04d} samples={len(toks)} cleaned={bool(cfg.export_cleaned_bev)} -> {path}")
        files.append(path)
        toks, bevs, bevs_clean, valids = [], [], [], []
        shard += 1
    ...
    for batch in loader:
        ...
        for tok, bev, v in zip(batch["sample_tokens"], pred, valid):
            ...
            bevs.append(bev)
            if bool(cfg.export_cleaned_bev):
                bevs_clean.append(clean_bev_prediction_np(bev, cfg))
            else:
                bevs_clean.append(bev)
            valids.append(v)
...
def main():
    ...
    print("=" * 88)
    print("v16.4.1 HIGH-RES DEPTH-SUPERVISED raw-image LSS BEV cache builder with background suppression")
    ...
    best_loss = math.inf
    best_path = os.path.join(cfg.out_dir, "best_v16_4_1_highres_depthsup_bgclean_real_image_lss_bev.pt")
    history = []
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        print(f"[epoch {epoch:03d}] train_loss={tr['loss']:.5f} val_loss={va['loss']:.5f} depth={tr['depth']:.5f} pts={tr['depth_pts']:.1f} bce={tr['bce']:.5f} focal={tr['focal']:.5f} dice={tr['dice']:.5f} edge={tr['edge']:.5f} boundary={tr['boundary']:.5f} sobel={tr['sobel']:.5f} thin={tr['thin']:.5f} bg_far={tr['bg_far']:.5f} bg_all={tr['bg_all']:.5f}")
    ...
    manifest = {
        "version": "v16.4.1_highres_depthsup_bgclean_real_image_lss_bev_cache",
        "description": "v16.4.1 high-resolution depth-supervised raw image LSS BEV cache with hard background false-positive suppression, thin-structure loss, and optional cleaned BEV export; default 320x512, D=48/64 env, hidden=192, BEV=120x90",
        "config": asdict(cfg),
        ...
        "clean_bev_key": "image_bev_feat_clean",
        "valid_key": "image_bev_valid",
        "use_cleaned_bev_hint": True,
        "sample_token_key": "sample_tokens",
    }
    manifest_path = os.path.join(cfg.out_dir, "image_bev_cache_manifest_v16_4_1.json")
    with open(manifest_path, "w", encoding="utf-8") as f:
        json.dump(manifest, f, indent=2, ensure_ascii=False)
    with open(os.path.join(cfg.out_dir, "train_history_v16_4_1.json"), "w", encoding="utf-8") as f:
