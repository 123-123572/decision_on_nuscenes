...
# -*- coding: utf-8 -*-
"""
Full v16.3 depth-supervised real-image BEV planner on nuScenes, built from v16.3 depth-supervised image-BEV planner.

This is a self-contained v9.5 pipeline built on the v9.2/v8.7 planner, upgraded with geometry-aligned visual tokens:

Stage 1 / v16.3 base model:
    Map-Agent-Ego token fusion planner
    + cached v16.3 depth-supervised real-image LSS BEV object-aligned fusion
    + class calibration
...
class Config:
    ...
    base_save_subdir: str = os.getenv("BASE_SAVE_SUBDIR", "v16_3_base")
    base_best_model_name: str = "best_v16_3_base_model.pt"
    ...
    save_dir: str = "/home/ubuntu22/decision_on_nuscenes/outputs_transformer_planning_v16_3_depthsup_image_bev"
    base_ckpt_path: str = os.getenv("BASE_CKPT_PATH", "/home/ubuntu22/decision_on_nuscenes/outputs_transformer_planning_v16_3_depthsup_image_bev/v16_3_base/best_v16_3_base_model.pt")
    ...
    bev_cache_manifest: str = os.getenv("IMAGE_BEV_CACHE_MANIFEST", os.getenv("BEV_CACHE_MANIFEST", "/home/ubuntu22/decision_on_nuscenes/outputs_v16_3_depthsup_real_image_lss_bev_cache_small/image_bev_cache_manifest_v16_3.json"))
    best_model_name: str = "best_score_model.pt"
    metrics_name: str = "metrics.json"
    model_type: str = "transformer_v16_3_depthsup_image_bev_full_pipeline"
...
def load_image_bev_cache(cache_manifest_path: str) -> Dict[str, Tuple[np.ndarray, np.ndarray]]:
    ...
    if not cache_manifest_path or not os.path.exists(cache_manifest_path):
        raise FileNotFoundError(
            ...
            "For v16.3, set IMAGE_BEV_CACHE_MANIFEST to image_bev_cache_manifest_v16_3.json."
        )
    manifest = load_json(cache_manifest_path)
    out: Dict[str, Tuple[np.ndarray, np.ndarray]] = {}
    ...
    for cache_file in cache_files:
        ...
        for tok, g, v in zip(tokens, grid, valid):
            ...
            if g.shape != (CFG.bev_channels, CFG.bev_h, CFG.bev_w):
                fixed = np.zeros((CFG.bev_channels, CFG.bev_h, CFG.bev_w), dtype=np.float32)
                c = min(CFG.bev_channels, g.shape[0])
                h = min(CFG.bev_h, g.shape[1])
                w = min(CFG.bev_w, g.shape[2])
                fixed[:c, :h, :w] = g[:c, :h, :w]
                g = fixed
...
class MapAgentEgoPlanner(nn.Module):
    def __init__(
        ...
        # v16.3 depth-supervised image-BEV adapters. Input is sampled raw BEV channel vector [C]
...
def train_v8_3_base_if_needed(
    ...
    print("\n" + "=" * 88)
    print("Stage 1: training v16.3 depth-supervised image-BEV base model inside this script")
    ...
    for epoch in range(1, CFG.base_epochs + 1):
        ...
        print(
            f"[v12.0 base] Epoch {epoch:03d} | TrainLoss {train_stats['loss']:.4f} | "
        ...
        if improved:
            ...
            ckpt_obj = {
                ...
                "model_type": "transformer_v16_3_depthsup_image_bev_base",
                "config": asdict(CFG),
            ...
            print(f"🔥 Saved ADE-aware v16.3 depth-supervised image-BEV base at epoch {epoch} (Macro-F1={macro_f1:.4f}, ADE={ade:.4f})")
        else:
            patience_counter += 1

        if patience_counter >= CFG.base_early_stop_patience:
            print(f"\n[v12.0 base] Early stopping triggered at epoch {epoch}.")
            break
...
def main():
    ...
    print("=" * 88)
    print("Training: train_transformer_planning_v16_3_depthsup_image_bev_planner_full_pipeline")
    ...
    if need_bev_cache:
        print("Loading v16.3 image-BEV cache for object-aligned / dense BEV fusion...")
    ...
    print(f"Frozen base      : v16.3 Depth-Supervised Image-BEV planner")
