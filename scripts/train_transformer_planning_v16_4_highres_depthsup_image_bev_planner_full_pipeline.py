...
# -*- coding: utf-8 -*-
"""
Full v16.4 high-res depth-supervised real-image BEV planner on nuScenes, built from v16.4 high-res depth-supervised image-BEV planner.

This is a self-contained v9.5 pipeline built on the v9.2/v8.7 planner, upgraded with geometry-aligned visual tokens:

Stage 1 / v16.4 base model:
    Map-Agent-Ego token fusion planner
    + cached v16.4 high-resolution depth-supervised real-image LSS BEV object-aligned fusion
    + class calibration
    + intent auxiliary head
    + behavior-conditioned mode queries

Stage 2 / v16.4 score-only calibration:
...
Default behavior:
    If BASE_CKPT_PATH points to a v16.4 base checkpoint, skip Stage 1 and run score calibration directly.
...
class Config:
    ...
    base_save_subdir: str = os.getenv("BASE_SAVE_SUBDIR", "v16_4_base")
    base_best_model_name: str = "best_v16_4_base_model.pt"
    ...
    save_dir: str = "/home/ubuntu22/decision_on_nuscenes/outputs_transformer_planning_v16_4_highres_depthsup_image_bev"
    base_ckpt_path: str = os.getenv("BASE_CKPT_PATH", "/home/ubuntu22/decision_on_nuscenes/outputs_transformer_planning_v16_4_highres_depthsup_image_bev/v16_4_base/best_v16_4_base_model.pt")
    ...
    bev_cache_manifest: str = os.getenv("IMAGE_BEV_CACHE_MANIFEST", os.getenv("BEV_CACHE_MANIFEST", "/home/ubuntu22/decision_on_nuscenes/outputs_v16_4_highres_depthsup_real_image_lss_bev_cache/image_bev_cache_manifest_v16_4.json"))
    best_model_name: str = "best_score_model.pt"
    metrics_name: str = "metrics.json"
    model_type: str = "transformer_v16_4_highres_depthsup_image_bev_full_pipeline"
...
def load_image_bev_cache(cache_manifest_path: str) -> Dict[str, Tuple[np.ndarray, np.ndarray]]:
    ...
    if not cache_manifest_path or not os.path.exists(cache_manifest_path):
        raise FileNotFoundError(
            ...
            "For v16.4, set IMAGE_BEV_CACHE_MANIFEST to image_bev_cache_manifest_v16_4.json."
        )
    manifest = load_json(cache_manifest_path)
    out: Dict[str, Tuple[np.ndarray, np.ndarray]] = {}
...
def train_v8_3_base_if_needed(
    ...
    """Return a v16.4 base checkpoint dict. Train v16.4 base first if requested or missing."""
    ...
    if not should_train and os.path.exists(load_path):
        print(f"Loading existing v16.4 base checkpoint: {load_path}")
        ckpt = torch.load(load_path, map_location=CFG.device, weights_only=False)
        return ckpt, load_path, {"trained": False, "path": load_path}

    if not should_train:
        raise FileNotFoundError(
            f"base v16.4 base checkpoint not found: {CFG.base_ckpt_path}\n"
            "Either set BASE_CKPT_PATH correctly, or set RUN_BASE_TRAIN=1."
        )

    print("\n" + "=" * 88)
    print("Stage 1: training v16.4 high-res depth-supervised image-BEV base model inside this script")
    ...
    for epoch in range(1, CFG.base_epochs + 1):
        ...
        print(
            ...
            f"Score {train_stats['loss_score']:.4f} | ValLoss {val_metrics['loss']:.4f} | "
        ...
        if improved:
            ...
            ckpt_obj = {
                ...
                "model_type": "transformer_v16_4_highres_depthsup_image_bev_base",
                "config": asdict(CFG),
            ...
            print(f"🔥 Saved ADE-aware v16.4 high-res depth-supervised image-BEV base at epoch {epoch} (Macro-F1={macro_f1:.4f}, ADE={ade:.4f})")
        else:
            patience_counter += 1
...
def main():
    ...
    print("=" * 88)
    print("Training: train_transformer_planning_v16_4_depthsup_image_bev_planner_full_pipeline")
    ...
    if need_bev_cache:
        print("Loading v16.4 image-BEV cache for object-aligned / dense BEV fusion...")
    ...
    print(f"Frozen base      : v16.4 High-Res Depth-Supervised Image-BEV planner")
