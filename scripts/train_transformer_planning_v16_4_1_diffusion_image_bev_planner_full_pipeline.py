...
# -*- coding: utf-8 -*-
"""
Full v16.4.1 + Diffusion trajectory-generation planner on nuScenes.

This version keeps the v16.4.1 high-res depth-supervised bg-cleaned real-image BEV encoder/fusion path,
but replaces the direct linear trajectory regression head with a conditional DDPM/DDIM trajectory generator.

This is a self-contained v9.5 pipeline built on the v9.2/v8.7 planner, upgraded with geometry-aligned visual tokens:

Stage 1 / v16.4.1 base model:
    Map-Agent-Ego token fusion planner
    + cached v16.4.1 high-resolution depth-supervised bg-cleaned real-image LSS BEV object-aligned fusion
    + class calibration
    + intent auxiliary head
    + behavior-conditioned mode queries

Optional Stage 2 / score-only calibration:
    Load or train the v16.4 base model, freeze it, and train only a tiny
    score refiner to reduce selected-vs-oracle trajectory gap without
    corrupting classification or trajectory generation.

Default behavior:
    If BASE_CKPT_PATH points to a v16.4.1 base checkpoint, skip Stage 1 and run score calibration directly.
...
4) Replaces direct trajectory regression with conditional diffusion denoising over [K,T,2] trajectories.
5) Keeps soft score supervision, map loss, diversity loss, comfort loss and simple collision loss.
...
class Config:
    ...
    use_cleaned_image_bev: bool = os.getenv("USE_CLEANED_IMAGE_BEV", "1").lower() in {"1", "true", "yes", "y", "on"}
    ...
    # v17.0 / v16.4-diffusion: conditional DDPM/DDIM trajectory generation.
    # Trajectory target is still normalized by traj_scale, so diffusion operates in a stable small range.
    use_diffusion_traj: bool = os.getenv("USE_DIFFUSION_TRAJ", "1").lower() in {"1", "true", "yes", "y", "on"}
    diffusion_train_timesteps: int = int(os.getenv("DIFFUSION_TRAIN_TIMESTEPS", "100"))
    diffusion_sample_steps: int = int(os.getenv("DIFFUSION_SAMPLE_STEPS", "20"))
    diffusion_sample_steps_train: int = int(os.getenv("DIFFUSION_SAMPLE_STEPS_TRAIN", "0"))
    diffusion_time_dim: int = int(os.getenv("DIFFUSION_TIME_DIM", "64"))
    diffusion_hidden_dim: int = int(os.getenv("DIFFUSION_HIDDEN_DIM", "256"))
    diffusion_beta_start: float = float(os.getenv("DIFFUSION_BETA_START", "1e-4"))
    diffusion_beta_end: float = float(os.getenv("DIFFUSION_BETA_END", "2e-2"))
    diffusion_x0_clip: float = float(os.getenv("DIFFUSION_X0_CLIP", "3.0"))
    diffusion_sample_noise_scale: float = float(os.getenv("DIFFUSION_SAMPLE_NOISE_SCALE", "1.0"))
    lambda_diffusion: float = float(os.getenv("LAMBDA_DIFFUSION", "2.0"))
    lambda_x0_recon: float = float(os.getenv("LAMBDA_X0_RECON", "0.40"))
    run_score_calibration: bool = os.getenv("RUN_SCORE_CALIBRATION", "0").lower() in {"1", "true", "yes", "y", "on"}

    # v9.2 score-only calibration losses.
    lambda_score_soft: float = 1.0
    lambda_score_rank: float = 0.10
    ...
    score_refiner_hidden_dim: int = 64
    ...
    # Full-pipeline control. Default trains v16.4 base if no v9.5 checkpoint exists.
    ...
    base_save_subdir: str = os.getenv("BASE_SAVE_SUBDIR", "v16_4_1_diffusion_base")
    base_best_model_name: str = "best_v16_4_1_diffusion_base_model.pt"
    ...
    save_dir: str = "/home/ubuntu22/decision_on_nuscenes/outputs_transformer_planning_v16_4_1_diffusion_bgclean_highres_depthsup_image_bev"
    base_ckpt_path: str = os.getenv("BASE_CKPT_PATH", "/home/ubuntu22/decision_on_nuscenes/outputs_transformer_planning_v16_4_1_diffusion_bgclean_highres_depthsup_image_bev/v16_4_1_diffusion_base/best_v16_4_1_diffusion_base_model.pt")
    ...
    best_model_name: str = "best_diffusion_model.pt"
    metrics_name: str = "metrics.json"
    model_type: str = "transformer_v16_4_1_diffusion_bgclean_highres_depthsup_image_bev_full_pipeline"
...
class SinusoidalTimeEmbedding(nn.Module):
    ...
    def forward(self, t: torch.Tensor) -> torch.Tensor:
        # t: any shape, integer or float diffusion step. Returns [*, dim].
        half = self.dim // 2
        device = t.device
        dtype = torch.float32
        freq = torch.exp(
...
class ConditionalTrajectoryDenoiser(nn.Module):
    """Per-mode conditional denoiser for normalized trajectories.

    Input:
        x_t       [B,K,T,2]
        t         [B,K]
        cond      [B,K,H+C] from Transformer mode feature + behavior probability
    Output:
        predicted Gaussian noise epsilon [B,K,T,2]
    """
    def __init__(self, traj_dim: int, cond_dim: int, time_dim: int, hidden_dim: int, dropout: float):
        super().__init__()
        self.time_emb = SinusoidalTimeEmbedding(time_dim)
        self.net = nn.Sequential(
            nn.Linear(traj_dim + cond_dim + time_dim, hidden_dim),
            nn.LayerNorm(hidden_dim),
            nn.GELU(),
            nn.Dropout(dropout),
    ...
    def forward(self, x_t: torch.Tensor, t: torch.Tensor, cond: torch.Tensor) -> torch.Tensor:
        B, K, T, D = x_t.shape
        x_flat = x_t.reshape(B, K, T * D)
        te = self.time_emb(t).to(dtype=x_flat.dtype)
        inp = torch.cat([x_flat, cond, te], dim=-1)
        eps = self.net(inp.reshape(B * K, -1)).reshape(B, K, T, D)
        return eps


class DiffusionTrajectoryHead(nn.Module):
    """Conditional DDPM training + DDIM sampling for K-mode future trajectories.

    This is deliberately lightweight: your 8GB laptop GPU cannot afford a giant
    U-Net here. The heavy lifting stays in the v16.4 token encoder; diffusion only
    models the trajectory distribution conditioned on each mode query.
    """
    def __init__(self, future_steps: int, future_dim: int, cond_dim: int, cfg: Config, dropout: float):
        super().__init__()
        self.future_steps = future_steps
        self.future_dim = future_dim
        self.traj_dim = future_steps * future_dim
        self.num_train_timesteps = int(cfg.diffusion_train_timesteps)
        self.x0_clip = float(cfg.diffusion_x0_clip)
        betas = torch.linspace(float(cfg.diffusion_beta_start), float(cfg.diffusion_beta_end), self.num_train_timesteps)
        alphas = 1.0 - betas
        ...
        self.denoiser = ConditionalTrajectoryDenoiser(
            ...
            cond_dim=cond_dim,
            time_dim=int(cfg.diffusion_time_dim),
            hidden_dim=int(cfg.diffusion_hidden_dim),
            dropout=dropout,
        )

    def _extract_alpha_bar(self, t: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
        ab = self.alpha_bars.to(device=x.device, dtype=x.dtype)[t.long().clamp(0, self.num_train_timesteps - 1)]
        return ab.view(*t.shape, 1, 1)

    def predict_x0_from_eps(self, x_t: torch.Tensor, t: torch.Tensor, eps: torch.Tensor) -> torch.Tensor:
        ab = self._extract_alpha_bar(t, x_t)
        x0 = (x_t - torch.sqrt((1.0 - ab).clamp_min(1e-8)) * eps) / torch.sqrt(ab.clamp_min(1e-8))
        return torch.clamp(x0, -self.x0_clip, self.x0_clip)

    def training_forward(self, cond: torch.Tensor, x0_target: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        B, K, T, D = x0_target.shape
        t = torch.randint(0, self.num_train_timesteps, (B, K), device=x0_target.device)
        noise = torch.randn_like(x0_target)
        ab = self._extract_alpha_bar(t, x0_target)
        x_t = torch.sqrt(ab) * x0_target + torch.sqrt((1.0 - ab).clamp_min(1e-8)) * noise
        eps_pred = self.denoiser(x_t, t, cond)
        x0_pred = self.predict_x0_from_eps(x_t, t, eps_pred)
        loss_diff = F.mse_loss(eps_pred, noise)
        loss_x0 = F.smooth_l1_loss(x0_pred, x0_target)
        return x0_pred, {"loss_diffusion": loss_diff, "loss_x0_recon": loss_x0}

    @torch.no_grad()
    def sample(self, cond: torch.Tensor, sample_steps: int, noise_scale: float = 1.0) -> torch.Tensor:
        B, K, _ = cond.shape
        device = cond.device
        dtype = cond.dtype
        steps = max(1, int(sample_steps))
        # Uniformly spaced DDIM steps. eta=0: deterministic reverse update once initial noise is sampled.
        t_seq = torch.linspace(self.num_train_timesteps - 1, 0, steps, device=device).long()
        x = torch.randn(B, K, self.future_steps, self.future_dim, device=device, dtype=dtype) * float(noise_scale)
        for i, t_scalar in enumerate(t_seq):
            t = torch.full((B, K), int(t_scalar.item()), device=device, dtype=torch.long)
            ...
            if i == len(t_seq) - 1:
                ...
                t_next = t_seq[i + 1]
                ab_next = self.alpha_bars.to(device=device, dtype=dtype)[t_next].view(1, 1, 1, 1)
                x = torch.sqrt(ab_next) * x0 + torch.sqrt((1.0 - ab_next).clamp_min(1e-8)) * eps
        return torch.clamp(x, -self.x0_clip, self.x0_clip)
...
class MapAgentEgoPlanner(nn.Module):
    def __init__(
        ...
        self.diffusion_traj_head = DiffusionTrajectoryHead(
            future_steps=future_steps,
            future_dim=future_dim,
            cond_dim=hidden_dim + num_classes,
            cfg=CFG,
            dropout=dropout,
        )
        # Emergency fallback for ablation only: USE_DIFFUSION_TRAJ=0 restores old direct regression.
    ...
    def forward(self, hist, agents, agent_valid, agent3d, agent3d_valid, bev_grid, bev_valid, maps, map_valid, camera_feat, camera_valid,
                proj_agent_feat, proj_agent_valid, proj_map_feat, proj_map_valid,
                proj_corridor_feat, proj_corridor_valid, y_traj: Optional[torch.Tensor] = None, y_cls: Optional[torch.Tensor] = None):
        B = hist.size(0)
        ...
        traj_cond = torch.cat([mode_feat, cls_expand], dim=-1)

        diffusion_aux: Dict[str, torch.Tensor] = {}
        if CFG.use_diffusion_traj:
            if self.training and y_traj is not None and y_cls is not None:
                mode_targets = build_mode_targets(y_traj, y_cls)
                traj, diffusion_aux = self.diffusion_traj_head.training_forward(traj_cond, mode_targets)
            else:
                traj = self.diffusion_traj_head.sample(
                    traj_cond,
                    sample_steps=CFG.diffusion_sample_steps,
                    noise_scale=CFG.diffusion_sample_noise_scale,
                )
        else:
            traj = self.traj_head(traj_cond).view(B, self.K, self.future_steps, self.future_dim)
        ...
        if diffusion_aux:
            return cls_logits, traj, scores, intent_pred, diffusion_aux
        return cls_logits, traj, scores, intent_pred

# ============================================================
# v16.4 base training losses / metrics / train loop
# ============================================================
...
def base_compute_total_loss(model, batch, cls_criterion, device: str):
    ...
    model_out = model(
        hist, agents, agent_valid, agent3d, agent3d_valid, bev_grid, bev_valid, maps, map_valid,
        camera_feat, camera_valid, proj_agent_feat, proj_agent_valid, proj_map_feat, proj_map_valid,
        proj_corridor_feat, proj_corridor_valid,
        y_traj=y_traj if model.training else None,
        y_cls=y_cls if model.training else None,
    )
    diffusion_aux: Dict[str, torch.Tensor] = {}
    if isinstance(model_out, tuple) and len(model_out) == 5:
        cls_logits, traj_pred, scores, intent_pred, diffusion_aux = model_out
    else:
        cls_logits, traj_pred, scores, intent_pred = model_out
    ...
    loss_diffusion = diffusion_aux.get("loss_diffusion", traj_pred.new_tensor(0.0))
    loss_x0_recon = diffusion_aux.get("loss_x0_recon", traj_pred.new_tensor(0.0))
    ...
    loss = (
        ...
        + CFG.lambda_diffusion * loss_diffusion
        + CFG.lambda_x0_recon * loss_x0_recon
    )
    return {
        "loss": loss,
        "loss_cls": loss_cls,
...
def base_train_one_epoch(model, loader, cls_criterion, optimizer, device: str):
    model.train()
    meters = {k: 0.0 for k in ["loss", "loss_cls", "loss_stop", "loss_traj_mode", "loss_traj_gt", "loss_score", "loss_map", "loss_div", "loss_comfort", "loss_collision", "loss_intent", "loss_diffusion", "loss_x0_recon"]}
    total = 0
    for batch in loader:
        optimizer.zero_grad()
...
def train_v8_3_base_if_needed(
    ...
    print("\n" + "=" * 88)
    print("Stage 1: training v16.4.1 bg-clean high-res depth-supervised image-BEV base model inside this script")
    ...
    for epoch in range(1, CFG.base_epochs + 1):
        ...
        print(
            f"[v16.4 base] Epoch {epoch:03d} | TrainLoss {train_stats['loss']:.4f} | "
            f"Cls {train_stats['loss_cls']:.4f} | TrajGT {train_stats['loss_traj_gt']:.4f} | "
            f"Score {train_stats['loss_score']:.4f} | Diff {train_stats.get('loss_diffusion', 0.0):.4f} | ValLoss {val_metrics['loss']:.4f} | "
        ...
        if improved:
            ...
            ckpt_obj = {
                ...
                "model_type": "transformer_v16_4_1_diffusion_bgclean_highres_depthsup_image_bev_base",
                "config": asdict(CFG),
            ...
            print(f"🔥 Saved ADE-aware v16.4.1 bg-clean high-res depth-supervised image-BEV base at epoch {epoch} (Macro-F1={macro_f1:.4f}, ADE={ade:.4f})")
        else:
            patience_counter += 1

        if patience_counter >= CFG.base_early_stop_patience:
            print(f"\n[v16.4 base] Early stopping triggered at epoch {epoch}.")
            break
...
class ScoreOnlyCalibrator(nn.Module):
    """
    v9.5: freeze the v16.4 base planner, then train only a tiny score calibrator.
    ...
    def __init__(self, base_model: MapAgentEgoPlanner, quality_dim: int = 12, hidden_dim: int = 64, dropout: float = 0.10):
        ...
        # Start exactly from the v16.4 base score behavior. Training begins as a safe no-op.
...
def score_calibration_losses(traj_pred: torch.Tensor, y_traj: torch.Tensor, scores: torch.Tensor, base_scores: torch.Tensor):
    ...
    # Keep calibration conservative; do not let delta dominate the already-trained v16.4 base scores.
...
@torch.no_grad()
def save_base_traj_visualizations(model, loader, device, save_dir, num_samples=12):
    model.eval()
    os.makedirs(save_dir, exist_ok=True)
    saved = 0
    for batch in loader:
        hist, agents, agent_valid, agent3d, agent3d_valid, bev_grid, bev_valid, maps, map_valid, y_cls, y_traj, _, _, _, camera_feat, camera_valid, proj_agent_feat, proj_agent_valid, proj_map_feat, proj_map_valid, proj_corridor_feat, proj_corridor_valid = unpack_batch(batch, device)
        cls_logits, traj_pred, scores, _ = model(
            hist, agents, agent_valid, agent3d, agent3d_valid, bev_grid, bev_valid, maps, map_valid,
            camera_feat, camera_valid, proj_agent_feat, proj_agent_valid, proj_map_feat, proj_map_valid,
            proj_corridor_feat, proj_corridor_valid,
        )
        ...
        for i in range(pred_plot.shape[0]):
            plt.figure(figsize=(5, 5))
            # Draw every generated mode faintly before selected mode.
            all_modes = (traj_pred[i] * CFG.traj_scale).cpu().numpy()
            for k in range(all_modes.shape[0]):
                plt.plot(all_modes[k, :, 0], all_modes[k, :, 1], marker=".", alpha=0.35, label=f"mode{k}" if k == 0 else None)
            plt.plot(gt_plot[i, :, 0], gt_plot[i, :, 1], marker="o", linewidth=2.0, label="GT")
            plt.plot(pred_plot[i, :, 0], pred_plot[i, :, 1], marker="x", linewidth=2.0, label="Diffusion Selected")
            plt.scatter([0.0], [0.0], marker="s", s=80, label="Ego")
            plt.axis("equal")
            plt.grid(True)
            plt.legend()
            plt.xlabel("x_local (m)")
            plt.ylabel("y_local (m)")
            plt.title(f"diffusion_sample_{saved:03d}")
            plt.savefig(os.path.join(save_dir, f"diffusion_traj_{saved:03d}.png"), bbox_inches="tight")
            plt.close()
            saved += 1
            if saved >= num_samples:
                return
...
def main():
    ...
    print("=" * 88)
    print("Training: train_transformer_planning_v16_4_1_diffusion_bgclean_depthsup_image_bev_planner_full_pipeline")
    ...
    if not CFG.run_score_calibration:
        base_model = build_base_model().to(CFG.device)
        base_model.load_state_dict(ckpt["model_state_dict"], strict=True)
        base_metrics = base_evaluate(base_model, val_loader, cls_criterion, CFG.device)
        traj_vis_dir = os.path.join(CFG.save_dir, "diffusion_traj_vis")
        save_base_traj_visualizations(base_model, val_loader, CFG.device, traj_vis_dir, num_samples=12)
        metrics = {
            "config": asdict(CFG),
            ...
            "final_val_loss": float(base_metrics["loss"]),
            "final_val_acc": float(base_metrics["acc"]),
            "final_val_macro_f1": float(base_metrics["macro_f1"]),
            "final_val_ADE_selected": float(base_metrics["ADE_selected"]),
            "final_val_FDE_selected": float(base_metrics["FDE_selected"]),
            "final_val_ADE_oracle": float(base_metrics["ADE_oracle"]),
            "final_val_FDE_oracle": float(base_metrics["FDE_oracle"]),
            "final_val_ADE_gap": float(base_metrics["ADE_selected"] - base_metrics["ADE_oracle"]),
            "final_precisions": {ID_TO_CLASS[i]: float(p) for i, p in enumerate(base_metrics["precisions"])},
            "final_recalls": {ID_TO_CLASS[i]: float(r) for i, r in enumerate(base_metrics["recalls"])},
            "final_f1s": {ID_TO_CLASS[i]: float(f) for i, f in enumerate(base_metrics["f1s"])},
            "confusion_matrix": base_metrics["cm"],
            "traj_vis_dir": traj_vis_dir,
        }
        with open(metrics_path, "w", encoding="utf-8") as f:
            json.dump(metrics, f, indent=2, ensure_ascii=False)
        # Save a final inference checkpoint in the same structure as the old best_model.pt.
        torch.save({
            "model_state_dict": base_model.state_dict(),
        ...
        print("\n========== Final Diffusion Summary ==========")
        print(f"Final Acc        : {base_metrics['acc']:.4f}")
        print(f"Final MacroF1    : {base_metrics['macro_f1']:.4f}")
        print(f"Final ADE_sel    : {base_metrics['ADE_selected']:.4f}")
        print(f"Final FDE_sel    : {base_metrics['FDE_selected']:.4f}")
        print(f"Final ADE_orc    : {base_metrics['ADE_oracle']:.4f}")
        print(f"Final FDE_orc    : {base_metrics['FDE_oracle']:.4f}")
        print(f"Final ADE_gap    : {base_metrics['ADE_selected'] - base_metrics['ADE_oracle']:.4f}")
        print("Confusion Matrix [rows=true, cols=pred]:")
        for i, row in enumerate(base_metrics["cm"]):
            print(f"{ID_TO_CLASS[i]:>7}: {row}")
        print(f"\nDiffusion trajectory visualizations saved to: {traj_vis_dir}")
    ...
    print(f"Frozen base      : v16.4.1 BG-Clean High-Res Depth-Supervised Image-BEV planner")
