...
# -*- coding: utf-8 -*-
"""
Full v10.0 3D-agent + projection-aligned visual token fusion planner on nuScenes FULL dataset.
...
class Config:
    ...
    num_workers: int = 0
    early_stop_patience: int = int(os.getenv("EARLY_STOP_PATIENCE", "8"))
    ...
    # In v12.3 frozen-v10 BEV-adapter, USE_ANNOTATION_BEV should normally be 1 and USE_DENSE_BEV_TOKENS should normally be 0.
    ...
    # v12.3: train only BEV adapters + score refiner on top of frozen v10.
    lambda_adapter_traj: float = float(os.getenv("LAMBDA_ADAPTER_TRAJ", "0.50"))
    lambda_adapter_score: float = float(os.getenv("LAMBDA_ADAPTER_SCORE", "1.00"))
    lambda_adapter_keep: float = float(os.getenv("LAMBDA_ADAPTER_KEEP", "0.00"))
    lambda_adapter_gate_l1: float = float(os.getenv("LAMBDA_ADAPTER_GATE_L1", "0.002"))
    adapter_grad_clip: float = float(os.getenv("ADAPTER_GRAD_CLIP", "2.0"))

    # v9.2 score-only calibration losses.
    lambda_score_soft: float = 1.0
    lambda_score_rank: float = float(os.getenv("LAMBDA_SCORE_RANK", "0.12"))
    ...
    score_delta_scale: float = float(os.getenv("SCORE_DELTA_SCALE", "0.35"))
    ...
    base_save_subdir: str = os.getenv("BASE_SAVE_SUBDIR", "v12_3_base_from_v10")
    base_best_model_name: str = "best_v12_3_base_from_v10_model.pt"
    ...
    save_dir: str = "/home/ubuntu22/decision_on_nuscenes/outputs_transformer_planning_v12_3_frozen_v10_bev_adapter"
    base_ckpt_path: str = os.getenv("BASE_CKPT_PATH", "/home/ubuntu22/decision_on_nuscenes/outputs_transformer_planning_v12_3_frozen_v10_bev_adapter/v12_3_base_from_v10/best_v12_3_base_from_v10_model.pt")
    init_ckpt_path: str = os.getenv("INIT_CKPT_PATH", os.getenv("V10_CKPT_PATH", "/home/ubuntu22/decision_on_nuscenes/outputs_transformer_planning_v10_0_clean_agent3d/best_model.pt"))
    ...
    model_type: str = "transformer_v12_3_frozen_v10_bev_adapter_full_pipeline"
...
class MapAgentEgoPlanner(nn.Module):
    def __init__(
        ...
        # v12.3 frozen-v10 BEV-adapter adapters. Input is sampled raw BEV channel vector [C]
        ...
        self.agent_bev_gate = nn.Parameter(torch.tensor(0.0, dtype=torch.float32))
        self.agent3d_bev_gate = nn.Parameter(torch.tensor(0.0, dtype=torch.float32))
        self.map_bev_gate = nn.Parameter(torch.tensor(0.0, dtype=torch.float32))
        self.corridor_bev_gate = nn.Parameter(torch.tensor(0.0, dtype=torch.float32))
...
def base_train_one_epoch(model, loader, cls_criterion, optimizer, device: str):
    model.train()
    meters = {k: 0.0 for k in ["loss", "loss_cls", "loss_stop", "loss_traj_mode", "loss_traj_gt", "loss_score", "loss_map", "loss_div", "loss_comfort", "loss_collision", "loss_intent"]}
    total = 0
    for batch in loader:
        optimizer.zero_grad()
...
def train_v8_3_base_if_needed(
    ...
    """v12.3: build a frozen-v10 anchor instead of re-training the whole base."""
    ...
    if (not CFG.run_base_train) and os.path.exists(CFG.base_ckpt_path):
        print(f"Loading existing v12.3 frozen-v10 anchor checkpoint: {CFG.base_ckpt_path}")
        ckpt = torch.load(CFG.base_ckpt_path, map_location=CFG.device, weights_only=False)
        return ckpt, CFG.base_ckpt_path, {"trained": False, "path": CFG.base_ckpt_path, "source": "existing_v12_3_anchor"}

    if not CFG.init_ckpt_path or not os.path.exists(CFG.init_ckpt_path):
        raise FileNotFoundError(
            f"v10 anchor checkpoint not found: {CFG.init_ckpt_path}\n"
            "Set INIT_CKPT_PATH or V10_CKPT_PATH to your v10.0_clean_agent3d best checkpoint.\n"
            "Do not use the old v8 checkpoint here; v12.3 is designed to preserve the v10 trunk."
        )

    print("\n" + "=" * 88)
    print("Stage 1: building v12.3 frozen-v10 anchor; no full base retraining")
    print(f"Anchor source ckpt: {CFG.init_ckpt_path}")
    print(f"Anchor save dir   : {base_dir}")
    print("Policy            : load compatible v10 tensors, keep BEV gates at 0, freeze trunk later")
    print("=" * 88)

    base_model = build_base_model().to(CFG.device)
    src_ckpt = torch.load(CFG.init_ckpt_path, map_location=CFG.device, weights_only=False)
    src_state = src_ckpt.get("model_state_dict", src_ckpt)
    cur_state = base_model.state_dict()
    compatible = {k: v for k, v in src_state.items() if k in cur_state and tuple(v.shape) == tuple(cur_state[k].shape)}
    skipped = [k for k in src_state.keys() if k not in compatible]
    missing = [k for k in cur_state.keys() if k not in compatible]
    base_model.load_state_dict(compatible, strict=False)

    with torch.no_grad():
        for name in ["agent_bev_gate", "agent3d_bev_gate", "map_bev_gate", "corridor_bev_gate"]:
            if hasattr(base_model, name):
                getattr(base_model, name).zero_()

    val_metrics = base_evaluate(base_model, val_loader, cls_criterion, CFG.device)
    print("\n========== v12.3 Anchor Check ==========")
    print(f"Loaded compatible tensors : {len(compatible)}")
    print(f"Missing/new tensors       : {len(missing)}")
    print(f"Skipped source tensors    : {len(skipped)}")
    print(f"Anchor Acc                : {val_metrics['acc']:.4f}")
    print(f"Anchor MacroF1            : {val_metrics['macro_f1']:.4f}")
    print(f"Anchor ADE_sel            : {val_metrics['ADE_selected']:.4f}")
    print(f"Anchor FDE_sel            : {val_metrics['FDE_selected']:.4f}")
    print(f"Anchor ADE_orc            : {val_metrics['ADE_oracle']:.4f}")
    print(f"Anchor FDE_orc            : {val_metrics['FDE_oracle']:.4f}")

    ckpt_obj = {
        "model_state_dict": base_model.state_dict(),
        "model_type": "transformer_v12_3_frozen_v10_bev_adapter_base",
        "config": asdict(CFG),
        "class_to_id": CLASS_TO_ID,
        "id_to_class": ID_TO_CLASS,
        "scaler": src_ckpt.get("scaler", scaler.state_dict()),
        "source_init_ckpt_path": CFG.init_ckpt_path,
        "compatible_tensor_count": len(compatible),
        "missing_tensor_count": len(missing),
        "skipped_source_tensor_count": len(skipped),
        "anchor_val_acc": float(val_metrics["acc"]),
        "anchor_val_macro_f1": float(val_metrics["macro_f1"]),
        "anchor_val_ADE_selected": float(val_metrics["ADE_selected"]),
        "anchor_val_FDE_selected": float(val_metrics["FDE_selected"]),
        "anchor_val_ADE_oracle": float(val_metrics["ADE_oracle"]),
        "anchor_val_FDE_oracle": float(val_metrics["FDE_oracle"]),
        "epoch": 0,
        "train_shapes": train_shapes,
        "val_shapes": val_shapes,
    }
    torch.save(ckpt_obj, generated_ckpt_path)
    print(f"Saved v12.3 frozen-v10 anchor to: {generated_ckpt_path}")

    ckpt = torch.load(generated_ckpt_path, map_location=CFG.device, weights_only=False)
    stage_metrics = {
        "trained": False,
        "path": generated_ckpt_path,
        "source": CFG.init_ckpt_path,
        "anchor_val_acc": float(val_metrics["acc"]),
        "anchor_val_macro_f1": float(val_metrics["macro_f1"]),
        "anchor_val_ADE_selected": float(val_metrics["ADE_selected"]),
        "anchor_val_FDE_selected": float(val_metrics["FDE_selected"]),
...
class FrozenV10BEVAdapterCalibrator(nn.Module):
    """
    v12.3: freeze v10 trunk, train only BEV adapters/gates plus score refiner.
    ...
    def __init__(self, base_model: MapAgentEgoPlanner, quality_dim: int = 12, hidden_dim: int = 64, dropout: float = 0.10):
        ...
        trainable_prefixes = (
            "bev_local_proj", "agent_bev_proj", "agent3d_bev_proj",
            "map_bev_proj", "corridor_bev_proj",
            "annotation_bev_encoder", "annotation_bev_norm",
        )
        trainable_names = {"agent_bev_gate", "agent3d_bev_gate", "map_bev_gate", "corridor_bev_gate"}
        for name, p in self.base.named_parameters():
            if name in trainable_names or any(name.startswith(pref) for pref in trainable_prefixes):
    ...
    def forward(self, hist, agents, agent_valid, agent3d, agent3d_valid, bev_grid, bev_valid, maps, map_valid, camera_feat, camera_valid,
        ...
        cls_prob = torch.softmax(cls_logits.detach(), dim=-1)
        cls_expand = cls_prob.unsqueeze(1).expand(-1, traj.size(1), -1)
        intent_expand = intent_pred.detach().unsqueeze(1).expand(-1, traj.size(1), -1)
        base_score_feat = base_scores.detach().unsqueeze(-1)
        refiner_input = torch.cat([base_score_feat, quality_feat.detach(), cls_expand, intent_expand], dim=-1)
...
def compute_total_loss(model: FrozenV10BEVAdapterCalibrator, batch, cls_criterion, device: str):
    hist, agents, agent_valid, agent3d, agent3d_valid, bev_grid, bev_valid, maps, map_valid, y_cls, y_traj, map_y_ref, map_ref_valid, intent_target, camera_feat, camera_valid, proj_agent_feat, proj_agent_valid, proj_map_feat, proj_map_valid, proj_corridor_feat, proj_corridor_valid = unpack_batch(batch, device)
    ...
    loss_cls = cls_criterion(cls_logits, y_cls)
    y_expand = y_traj.unsqueeze(1).expand_as(traj_pred)
    traj_err = ((traj_pred - y_expand) ** 2).sum(dim=-1).mean(dim=-1)
    loss_traj = traj_err.min(dim=1)[0].mean()
    gate_l1 = (model.base.agent_bev_gate.abs() + model.base.agent3d_bev_gate.abs() + model.base.map_bev_gate.abs() + model.base.corridor_bev_gate.abs())
    loss_keep = traj_pred.new_tensor(0.0)
    loss = CFG.lambda_adapter_score * loss_score + CFG.lambda_adapter_traj * loss_traj + CFG.lambda_adapter_keep * loss_keep + CFG.lambda_adapter_gate_l1 * gate_l1

    outputs = {
        "loss": loss,
        "loss_score": loss_score,
        "loss_adapter_traj": loss_traj,
        "loss_adapter_keep": loss_keep,
        "loss_gate_l1": gate_l1,
...
def train_one_epoch(model, loader, cls_criterion, optimizer, device: str):
    model.train()
    model.base.eval()
    meters = {k: 0.0 for k in ["loss", "loss_score", "loss_adapter_traj", "loss_adapter_keep", "loss_gate_l1", "loss_score_soft", "loss_score_rank", "loss_score_reg", "loss_cls"]}
    total = 0
    for batch in loader:
        optimizer.zero_grad()
        out = compute_total_loss(model, batch, cls_criterion, device)
        out["loss"].backward()
        torch.nn.utils.clip_grad_norm_([p for p in model.parameters() if p.requires_grad], max_norm=CFG.adapter_grad_clip)
        optimizer.step()
        bs = out["y_cls"].size(0)
        total += bs
        for k in meters:
...
def save_traj_visualizations(model, loader, device, save_dir, num_samples=12):
    ...
    for batch in loader:
        ...
        for i in range(pred_plot.shape[0]):
            ...
            plt.plot(pred_plot[i, :, 0], pred_plot[i, :, 1], marker="x", label="v12.3 Selected Pred")
...
def main():
    ...
    print("=" * 88)
    print("Training: train_transformer_planning_v12_3_frozen_v10_bev_adapter_full_pipeline")
    ...
    print(f"BEV fusion     : basic_agent={CFG.use_agent_bev_fusion}, agent3d={CFG.use_agent3d_bev_fusion}, map={CFG.use_map_bev_fusion}, corridor={CFG.use_corridor_bev_token}")
    print(f"Adapter losses : score={CFG.lambda_adapter_score}, traj={CFG.lambda_adapter_traj}, gate_l1={CFG.lambda_adapter_gate_l1}")
    print("=" * 88)
    ...
    if CFG.use_object_aligned_bev and not CFG.use_annotation_bev:
        raise RuntimeError(
            ...
            "Object-aligned BEV fusion would only receive zero BEV placeholders. "
            "Set USE_ANNOTATION_BEV=1 or disable USE_OBJECT_ALIGNED_BEV."
        )
    ...
    if CFG.use_annotation_bev:
        print("Loading v11.0 annotation-BEV cache...")
        bev_cache = load_annotation_bev_cache(CFG.bev_cache_manifest)
        print(f"Annotation BEV cache samples: {len(bev_cache)}")
    else:
    ...
    print(f"Annotation BEV    : {BEV_train.shape}, valid mean={BEVv_train.mean():.4f}")
    ...
    model = FrozenV10BEVAdapterCalibrator(base_model, quality_dim=CFG.score_quality_dim, hidden_dim=CFG.score_refiner_hidden_dim, dropout=CFG.score_refiner_dropout).to(CFG.device)
    ...
    print(f"Frozen base      : v12.3 Frozen-v10 BEV-Adapter planner")
    ...
    print("Goal             : preserve v10 trajectory ability, let zero-gated BEV adapters learn only helpful residuals")
    ...
    for epoch in range(1, CFG.epochs + 1):
        ...
        improved = False
        # v12.3: select the score-calibrated model by planning score, not ADE alone.
    ...
    best_model = FrozenV10BEVAdapterCalibrator(final_base, quality_dim=CFG.score_quality_dim, hidden_dim=CFG.score_refiner_hidden_dim, dropout=CFG.score_refiner_dropout).to(CFG.device)
